//Driver for 1D chemically structured model with canonical QS kinetics
//Solves the coupled equations
// n_t = D(u)n_{xx} n + epsilon*n_{uu} - (f(u,c)n)_u
// c_t = Dc c_{xx} - beta*c + alpha_0*ubar
// ubar = int_0^\infty u*n du
//

// Generic oomph-lib routines
#include "generic.h"
#include "StratifiedHelmholtzElement1D.h"
...
namespace GlobalParams
{
	...
	//kinetic parameters
	double a, L, K, lambda, rho_star;

	// regularization parameter
	double epsilon_reg;

	//swimming parameters
	double D1_star, D1_p_star, D_inf_0;	//swimming motility coefficient and it's derivative at u=u*, and a parameter representing either D_inf or D_0
	...
	void get_IC(const double& time, const Vector<double>& x, Vector<double>& sol)
	{
		...
		//double gaussian = std::exp(F/epsilon)
		double normalization = std::sqrt(lambda/(2*M_PI*epsilon_IC));
		double ss = rho_star*normalization*gaussian;	//steady-state structure

		// compute steady-state AI concentration from u_star
		double ce = alpha/beta_decay*rho_star*u_star;
		
		//random perturbation to steady-state
		double amplitude = 1e-2;
		double r = 2.0*(static_cast <double> (rand()) / static_cast <double> (RAND_MAX) - 0.5);	// random double between 0 and 1
		sol[0] = ss + amplitude*r*ss;
		r = 2.0*(static_cast <double> (rand()) / static_cast <double> (RAND_MAX) - 0.5);	// new random number
		sol[1] = ce + amplitude*r*ce;
		sol[2] = u_star*rho_star;
	}

	// function for evaluating the swimming coefficient
...
class UbarElement : public virtual GeneralisedElement
{
	...
	void compute_first_moment(Vector<double>& first_moments)	{
		...
		//go through all elements with fixed x. Note there there may be 
		//nodes within each element that have different values of x
		for(unsigned j=0; j<n_element_density_u; j++)	{
			...
			for(unsigned k=0; k<num_node; k++)	{	 
				...
				int integral_ind = int(s[1] + 1.0 + 0.5); // add an additional 0.1 to avoid truncation errors (range of local coords is [-1,1])
...
public:
	...
	void get_kinetics_density(const unsigned& ipt, const Vector<double>& x, double* kinetics) const		{
		...
		*kinetics = a + L*interpolated_c/(K + interpolated_c) - lambda*x[0]; //full nonlinear kinetics
	}
...
public:
	...
	void get_source_strat_helmholtz(const unsigned& ipt, const Vector<double>& x, double* source) const
	{
		...
		basis_fn[0] = xloc*(xloc-1.0)/2.0;	//zero at xloc = 0,1, one at xloc=-1
		basis_fn[1] = (1.0-xloc)*(1.0+xloc);	//zero at xloc = -1,1, one at xloc=0
		basis_fn[2] = xloc*(xloc+1.0)/2.0;	//zero at xloc = -1,0, one at xloc=1
		double ubar=0.0;
		for(unsigned j=0; j<3; j++)	{
			ubar += basis_fn[j]*external_data_pt(0)->value(j);
		}
		*source = alpha*ubar;
	}
	...
	void fill_in_contribution_to_residuals(Vector<double>& residuals)
	{
		...
		StratifiedHelmholtzElement1D::fill_in_contribution_to_residuals(residuals);
	}
		...
		const unsigned& num_el_u_fine, const unsigned& num_el_u_coarse, const unsigned& num_el_x) 
{
	...
	oomph_info << "Number of equations is " << assign_eqn_numbers(false) << std::endl;	
...
void FullDensityProblem<DENSITY_ELEMENT, HELM_ELEMENT>::create_ubar_elements()	{
	...
	Ubar_msh_pt->enable_output_of_halo_elements();
}

//==================Start of compute_solution_mass()==========================
// computes the total population in the density
...
double FullDensityProblem<DENSITY_ELEMENT, HELM_ELEMENT>::compute_solution_mass(int itime)	{
	...
	for(unsigned i=0; i<num_el; i++)	{
		...
		for(unsigned ipt=0; ipt<n_intpt; ipt++)	{
			...
			s[0] = el_pt->integral_pt()->knot(ipt,0); // arguments are (i,j) where i is the index of the integration poitn and j is the jth coordinate
			s[1] = el_pt->integral_pt()->knot(ipt,1);
	...
	// Loop over current & previous timesteps	 
		...
		double time=prev_time[itime];
   		...
   		for (unsigned i=0;i<num_nod_density;i++)	{
			...
			//sync IC across processors (required due to random number generation in perturbation)
			for(unsigned ii=0; ii<soln.size(); ii++)	{
				#ifdef OOMPH_HAS_MPI
   		...
   		for(unsigned i=0; i<num_nod_density; i++)	{
   			...
   			density_msh_pt()->node_pt(i)->set_value(itime,0,nval*rho_star*Lx/total_population);
   		}
		oomph_info << "total population is now " << compute_solution_mass(itime) << endl;

		//Loop over the nodes in the helmholtz mesh to set initial guess everywhere
   		for(unsigned i=0;i<num_nod_helm;i++)	{
			...
			//assign this initial condition to all nodes in the mesh with the same values of x
			//so that c is independent of u
   			double x_i = x[1];
			for(unsigned i2=0; i2<num_nod_helm;i2++)	{
				double xcurr = helm_msh_pt()->node_pt(i2)->x(1);
				if(abs(xcurr-x_i) < 0.05*dx)	{
					helm_msh_pt()->node_pt(i2)->set_value(itime, 0, soln[1]); // c is at index 0 at each node
				}
			}
		...
		unsigned n_ubardata = Ubar_msh_pt->nelement();
   		for(unsigned i=0; i<n_ubardata; i++)	{
...
double FullDensityProblem<DENSITY_ELEMENT, HELM_ELEMENT>::global_temporal_error_norm()
{
	double global_error = 0.0;	   
	 //Find out how many nodes there are in the density mesh
...
// there should be minimum 2 command line arguments, the first specifies the parameter values in a file,
// the second specifies the output file directory
int read_input_params_from_file()
{
	...
	std::string buffer;
	unsigned n_params = 20, num_params_assigned=0; // number of parameters that need to be assigned
	...
	while(std::getline(infile,buffer))	{//read in one line
		...
		if(pos != std::string::npos)	{ // found '=' in line
			...
			if(param_name=="a")		{a = val; 	 	}
				...
				return 0;// should never get here if input file is configured correctly
			}
...
int main(int argc, char* argv[])
 {
	...
	//set the error and warning streams as well
	...
	problem.set_initial_condition();	//note that the IC must be assigned before distribution when using random initial noise otherwise processors end up with conflicting initial data
	...
	if(CommandLineArgs::command_line_flag_has_been_set("--restart-file"))	{
		...
		if(MPI_Helpers::communicator_pt()->my_rank() == 0)	{
			t_out_file << std::setprecision(10) << problem.time_pt()->time() << endl; // output initial time to file
		}
	}
	//initialise dt for first timestep 
	double dt=problem.next_dt();
	
	// next time to record solution at
	...
	unsigned steps_btw_restarts= 2500;	//write restart file every steps_btw_restarts timesteps 
	unsigned steps = 0;	//number of steps taken thus far
	unsigned max_steps = 1e9;	//maximum number of steps before terminating
	// Timestepping loop
	while (problem.time_pt()->time()<t_max && steps < max_steps)
	 {
	   ...
	   //if(problem.time_pt()->time()>t_out && steps%10==0)
	   if(problem.time_pt()->time()>t_out)
	   {
		   problem.doc_solution();
		   t_out = problem.time_pt()->time() + dt_out; //next time at which solution should be doc'ed
		   //record the time in a separate output file
		   if(MPI_Helpers::communicator_pt()->my_rank() == 0)	{ //record only once
			   t_out_file << std::setprecision(10) << problem.time_pt()->time() << endl;
		   }
	   ...
	   steps_since_write++; 
	   steps++;	//update the count of steps taken
	   if(steps > max_steps)	{
		   oomph_info << "Reached maximum number of steps... terminating" << endl;
	   }
	  } // end of timestepping loop

	//write one last restart file before terminating
	problem.doc_solution();
	t_out_file << std::setprecision(10) << problem.time_pt()->time() << endl;
	problem.doc_restart();
	#ifdef OOMPH_HAS_MPI	
