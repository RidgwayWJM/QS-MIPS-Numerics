...
class StratifiedHelmholtzElement2D : public virtual QElement<3,3>
{
  public:
  //Constructor
  //initialises Source function pointer and pointer to decay coeff to null
  StratifiedHelmholtzElement2D() : QElement<3,3>() 
  {
  ...
  inline double interpolated_c_strat_helmholtz(const Vector<double>& s) const
  {
	  ...
	  {
		  interpolated_c += nodal_value(j, field_index_strat_helmholtz())*psi[j];
	  }
