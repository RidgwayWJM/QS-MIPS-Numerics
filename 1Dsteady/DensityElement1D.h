...
class DensityElement1D : public virtual QElement<2,3>
{
  public:
  //Constructor
  //initialises Source function pointer and pointer to decay coeff to null
  DensityElement1D() : QElement<2,3>() 
  {
	  Motility_fct_pt = 0; // pointer to diffusion coefficient
	  Kinetic_fct_pt = 0; //pointer to effective diffusion coefficient
  ...
  //function pointer to effective diffusion coefficient
  typedef void (*DensityMotilityFctPt) (const Vector<double>& x, double* D1);

  // Access function to effective diffusion coefficient
  DensityMotilityFctPt& motility_fct_pt()
  {
	  return Motility_fct_pt;
  }

  //Access function to effective diffusion coefficient (const version)
  ...
  inline double interpolated_n_density(const Vector<double>& s) const
  {
	  ...
	  {
		  interpolated_n += this->nodal_value(j, field_index_density())*psi[j];
	  }
  ...
  //evaluates the user-defined effective diffusion coefficient (if present) at global coordinate x, returns 1 if not specified
  inline virtual void get_motility_coeff(const unsigned& ipt, const Vector<double>& x, double* D1) const
  {
	  //return 1 if no coefficient has been specified
 ...
 protected:
  ...
  DensityKineticFctPt Kinetic_fct_pt;
  //pointer to effective diffusion coefficient
  DensityMotilityFctPt Motility_fct_pt;
